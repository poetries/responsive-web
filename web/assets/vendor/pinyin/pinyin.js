...
(function(PY){
	 ...
	 * 转换单个字
	 */
	function transform(word, opts){
		var rt = [''];

		!opts && (opts = {});

		if (word && word.length > 1) {
			return PY.transform(word);
		};
		...
		if(word.charCodeAt(0) <= 255) {
			...
			for(var i=0, l=DICTIONARY_HZ.length; i<l; i++){
				if(DICTIONARY_HZ[i].indexOf(word) >= 0) {
					// 是否继续识别多音字
					rt.push(DICTIONARY_PY[i]);
		...
		return typeof rt==='string' ? rt : rt.join('');
	}
	 ...
	 */
	PY.transform = function(str, opts){
		var rt = [], abbr = [];
